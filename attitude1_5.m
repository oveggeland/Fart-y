...
%% q reference
   ...
   tau = -400*eye(3)*w-20*q_thilde(2:4); % control law
   ...
   table(i,:) = [t q' phi theta psi w' tau' q_ref_euler];  % store data in table
...
tau     = table(:,12:14);
q_ref_tabl = rad2deg*table(:,15:17);
...
plot(t, theta, 'r');
plot(t, psi, 'g');
plot(t, q_ref_tabl(:,1), 'k');
plot(t, q_ref_tabl(:,2), 'y');
plot(t, q_ref_tabl(:,3), 'c');
hold off;
grid on;
